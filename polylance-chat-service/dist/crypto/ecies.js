...
export function recoverPublicKey(message, signature) {
    return EthCrypto.recoverPublicKey(signature, EthCrypto.hash.keccak256(message));
}
export async function createConversationKey(clientPubKey, freelancerPubKey) {
...
export async function decryptOwnKey(encryptedKeyString, privateKey) {
    const parsed = EthCrypto.cipher.parse(encryptedKeyString);
    return EthCrypto.decryptWithPrivateKey(privateKey, parsed);
}
export function encryptMessage(plaintext, symmetricKeyHex) {
...
export function decryptMessage(ciphertext, symmetricKeyHex) {
    const key = Buffer.from(symmetricKeyHex, "hex");
